...
public class TestWebApplicationFactory : WebApplicationFactory<Program>
{
    ...
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        ...
        {
            var descriptor = services.First(d => d.ServiceType == typeof(DbContextOptions<FitnessContext>));
            services.Remove(descriptor);
            services.AddDbContext<FitnessContext>(options =>
            {
                options.UseSqlite("DataSource=fitnesscenter_test.db");
            });
        });
        builder.UseEnvironment("Development");
    }
    public void InitializeDatabase(Action<FitnessContext> action)
    {
        using var scope = Services.CreateScope();
        using var db = scope.ServiceProvider.GetRequiredService<FitnessContext>();
    ...
    public Tout QueryDatabase<Tout>(Func<FitnessContext, Tout> query)
    {
        using var scope = Services.CreateScope();
        using var db = scope.ServiceProvider.GetRequiredService<FitnessContext>();
        return query(db);
