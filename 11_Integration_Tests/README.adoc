...
=== Arbeiten mit JsonElement (System.Text.Json)

Oft liefern API-Aufrufe (besonders bei POST, PUT oder DELETE) dynamische oder kleine Datenstrukturen zurück, für die sich das Anlegen eines eigenen DTOs nicht lohnt (z. B. `{ "id": 12 }`). In diesen Fällen liefert die Test-Factory ein `JsonElement` zurück, mit dem die "rohe" JSON-Antwort dynamisch ausgewertet werden kann.

Folgende Methoden sind für das Extrahieren von Daten in den `Assert`-Prüfungen wichtig:

`JsonElement GetProperty(string propertyName)`::
Sucht auf einem JSON-Objekt nach einer Eigenschaft mit dem angegebenen Namen und gibt deren Wert als neues `JsonElement` zurück. Damit navigiert man im JSON-Baum.
+
[source,csharp]
----
...
`int GetInt32()`::
Wandelt den aktuellen JSON-Zahlenwert in einen C#-`int` um. Nützlich zur Überprüfung von IDs oder Zählern.
+
[source,csharp]
----
Assert.Equal(12, content.GetProperty("id").GetInt32());
----

`string? GetString()`::
Liest den aktuellen JSON-String-Wert aus und gibt ihn als C#-`string` zurück.
+
[source,csharp]
----
Assert.Equal("Floor", content.GetProperty("contingentType").GetString());
----

`JsonElement.ArrayEnumerator EnumerateArray()`::
Wenn das `JsonElement` ein JSON-Array darstellt (`[ ... ]`), liefert diese Methode einen Enumerator, um durch die einzelnen Array-Elemente zu iterieren. Jedes Element ist wiederum ein `JsonElement`.
+
[source,csharp]
----
...
Wenn das `JsonElement` ein JSON-Objekt darstellt (`{ ... }`), kann hiermit durch alle Key-Value-Paare des Objekts iteriert werden. Jedes Element der Schleife ist ein `JsonProperty` mit einem Namen (`Name`) und einem Wert (`Value`). Dies ist besonders nützlich, um dynamische Validierungsfehler auszuwerten.
...
.link:{github_url}/Eventmanager/Eventmanager.Test/ContingentsControllerTests.cs[→ Eventmanager.Test/ContingentsControllerTests.cs]
[source,csharp]
----
...
public class ContingentsControllerTests : IDisposable
{
    ...
    public ContingentsControllerTests()
    {
        ...
        _factory.SubstituteService<EventContext>(
            opt => opt.UseSqlite(_connection));
        _factory.SubstituteService<TimeProvider>(
            opt => _timeProvider, ServiceLifetime.Singleton);
    }
    /// <summary>
    ...
    /// <summary>
    /// Checks whether POST /api/contingents returns HTTP 201 with ID
    ...
    public async Task CreateContingentReturns201Test()
    {
        ...
        Assert.True(_factory.QueryDatabase(
            db => db.Contingents.Any(c => c.Id == 2)));
        // Create returns a JSON: { id : 12 }. 
        // You can read the JSON with JsonElement Methods.
    ...
    public async Task DeleteContingentReturns204Test()
    {
        ...
        Assert.Equal(204, (int)response.StatusCode);
        Assert.False(_factory.QueryDatabase(
            db => db.Contingents.Any(c => c.Id == 1)));
    }
    /// <summary>
